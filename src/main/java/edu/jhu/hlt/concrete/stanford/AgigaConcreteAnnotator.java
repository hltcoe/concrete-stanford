 ...
 * annotations via an AgigaDocument, add these annotations and return a new
 * Communication
 */
@Deprecated
class AgigaConcreteAnnotator {
  ...
  private static final Logger logger = LoggerFactory
      .getLogger(AgigaConcreteAnnotator.class);
  ...
  public AgigaConcreteAnnotator(boolean setSpans, String language)
      throws IOException {
  ...
  SimpleEntry<EntityMentionSet, EntitySet> convertCoref(Communication in,
      AgigaDocument agigaDoc, List<Tokenization> tokenizations)
  ...
  void convertSection(Section section, AgigaDocument agigaDoc, int charOffset,
    ...
    if (section.isSetSentenceList()) {
      this.convertSentences(section, agigaDoc, charOffset, sb,
  ...
  private void convertSentences(Section in, AgigaDocument ad, int currOffset,
    ...
    // logger.debug("AgigaToken: " + at.getWord() + ": " + at.getCharOffBegin()
    // + " -> " + at.getCharOffEnd());
    // }
    // }
    ...
    for (int i = 0; i < n; i++) {
      ...
      Tokenization tokenization = this.ag.convertTokenization(asent,
          currOffset, preserveTokenTaggings);
      ...
      AgigaToken lastToken = asent.getTokens()
          .get(asent.getTokens().size() - 1);
      TextSpan compTS = new TextSpan(firstToken.getCharOffBegin(),
          lastToken.getCharOffEnd());
      boolean isValidCompTS = new ValidatableTextSpan(compTS).isValid();
      if (!isValidCompTS)
        throw new AnnotationException("Computed TextSpan was not valid: "
            + compTS.toString());
