 ...
 *
 */
public class StanfordAgigaPipe_ZipTest {
  ...
  public void test_numProcessedZip() throws TException, IOException {
      assertTrue("Expected 2 processed communications, got " + processedCommunications.size(),
                 processedCommunications.size() == 2);
  }
...
//   }
  ...
  public void testZipTest_sentText() throws TException, InvalidInputException, IOException, ConcreteException {
      String[] sentences = {
          ...
          "dissent, a charge the government has denied."};
      int sentIdx = 0;
      for(Section sect : processedCommunications.get(0).getSectionSegmentations().get(0).getSectionList()){
          if(sect.getSentenceSegmentation() != null) {
  ...
  public void testTwoZip_verifyTokensToFull() throws TException, InvalidInputException, IOException, ConcreteException {
      ...
      for(Communication pcomm : StanfordAgigaPipe_ZipTest.processedCommunications) {
          ...
          String docText = pcomm.getProcessedContent();
          int numEq = 0;
          int numTot = 0;
          for(Section nsect : pcomm.getSectionSegmentations().get(0).getSectionList()){
              ...
              for(Sentence nsent : nsect.getSentenceSegmentation().get(0).getSentenceList()){
                  for(Token token : nsent.getTokenizationList().get(0).getTokenList().getTokens()){
                      ...
                      if(!areEq) {
                          logger.warn("Communication " + pcomm.getId() + " (index = " + commIndex + 
                                      ") :: expected = ["+ token.getText() + "];" +
                                      "docText("+ tts +") = [" + substr + "]");
                      } else {
                          numEq++;
                      }
